...
from bot import parent_id, DOWNLOAD_DIR, IS_TEAM_DRIVE, INDEX_URL, USE_SERVICE_ACCOUNTS, BUTTON_FOUR_NAME, \
                ...
                DRIVES_NAMES, DRIVES_IDS, INDEX_URLS, EXTENSION_FILTER, IMAGE_URL
...
class GoogleDriveHelper:
    ...
    def __upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'Uploaded by Google Drive',
            'mimeType': mime_type,
        }
...
# Type File : File
    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""):
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            for file in response.get('files', []):
                ...
                if mime_type == "application/vnd.google-apps.folder":
                    ...
                    msg += f"📁 Name :- <code>{file.get('name')}</code><br>📥 Total Size :- Unknown<br>🏷 Type Files :- Folder / 🗂<br>🔗 Link :-<br>"
                    furl = short_url(furl)
                    ...
                    msg += f"📄 Name :- <code>{file.get('name')}</code><br>📥 Total Size :- {get_readable_file_size(int(file.get('size', 0)))}<br>🏷 Type Files :- File / 📄<br>🔗 Link :-<br>"
                    furl = short_url(furl)
                    msg += f"<b><a href={furl}>⚡️ Google Drive ⚡️</a></b>"
                    if INDEX_URLS[index] is not None:
                        if isRecur:
        ...
        buttons = ButtonMaker()
        buttons.buildbutton("🔎 Check ↗️", f"https://telegra.ph/{path[0]}")
